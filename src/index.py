...
def create_user():
    while True:
        name = input("Syötä haluamasi käyttäjänimi (vähintään 3 merkkiä). Palaa takaisin syöttämällä 'X'.")
        if name == "X":
            return
        if len(name) > 2:
            break
    ...
    number = False
    while not number:
        password = input("Syötä haluamasi salasana. Käytä vähintään 8 merkkiä, joista vähintään yksi on numero.")
        if len(password) < 8:
